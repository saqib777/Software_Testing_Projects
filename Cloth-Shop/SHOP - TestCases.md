...
36      | 25-Nov-25  | SHOP        | TC-INT-001    | Product API Integration         | Product Listing  | 1             | Load product listing page                 | API Call     | Product service endpoint        | GET /products               | Product grid populated with API data                                            |               |        |                                | 3 min
37      | 25-Nov-25  | SHOP        | TC-INT-002    | Cart API Integration            | Cart             | 1             | Add product to cart                       | API Call     | Cart service endpoint           | POST /cart with product ID  | Cart updates correctly; API response matches UI                                 |               |        |                                | 4 min
38      | 25-Nov-25  | SHOP        | TC-INT-003    | Checkout API Integration        | Checkout         | 1             | Proceed to checkout                       | API Call     | Checkout service endpoint       | POST /checkout              | Checkout page loads; API response confirms order creation                       |               |        | Demo site may stub checkout     | 5 min
39      | 25-Nov-25  | SHOP        | TC-INT-004    | Data Persistence in Cart        | Cart             | 1             | Refresh page after adding items           | Persistence  | Local storage/session storage   | Cart with items             | Cart state persists after refresh                                                |               |        |                                | 3 min
40      | 25-Nov-25  | SHOP        | TC-INT-005    | Component Communication         | UI/UX            | 1             | Add item, check cart icon update          | Component    | Product card + cart icon        | Product ID                  | Cart icon updates immediately after item added                                   |               |        |                                | 2 min
41      | 25-Nov-25  | SHOP        | TC-INT-006    | Error Handling in API Calls     | API              | 1             | Simulate API failure                      | Error        | Product/cart API                | Force 500 error             | UI shows error message; no crash                                                 |               |        |                                | 4 min
42      | 25-Nov-25  | SHOP        | TC-INT-007    | Cross‑Module Integration        | Navigation       | 1             | Add item, navigate to checkout            | Integration  | Product → Cart → Checkout       | Product ID                  | Item flows correctly from product to cart to checkout                           |               |        |                                | 6 min
